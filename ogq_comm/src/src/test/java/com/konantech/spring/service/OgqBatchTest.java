...
public class OgqBatchTest extends CommonTests {
    ...
    public void insert() throws Exception {
        ...
        item.setVideoUrl("http://s3.amazon.com/media/image/2017-06/te33033.mp4");
        item.setCallbackUrl("http://bgh.ogqcorp.com/video_tagging/callback/1sdv23d4234");
        item.setRequestId("1sdv23d4234");

        URL url = new URL(item.getVideoUrl());
        Map<String, String> map = new HashedMap();
        map.put("title", item.getRequestId());
        map.put("content", item.getRequestId());
        map.put("orifilename", FilenameUtils.getName(url.getPath()));
        map.put("filepath", item.getVideoUrl());
        ResponseEntity responseEntity = RestUtils.darc4add(map);
