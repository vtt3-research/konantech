...
#include "stdafx.h"
#include "KRestGoogleCloudService.h"
...
const pair<string, string> g_pairObjectLabel("POST", "/v1/images:annotate?");

CKRestGoogleCloudService::CKRestGoogleCloudService(void)
{
	m_nError = 0;
...
void CKRestGoogleCloudService::SetServiceInfo(string stdstrServiceUri, string stdstrKey, float fMinScore)
{
	m_stdstrServiceUri = stdstrServiceUri;
	m_stdstrKey = stdstrKey;
	m_fMinScore = fMinScore;
}

void CKRestGoogleCloudService::SetMaxLabelResult(int nMax)
{
	m_nMaxLabelResults = nMax;
}

int CKRestGoogleCloudService::LabelDetectionOnImage(string stdstrImageFilePath)
{
	...
	printf("request - %s(%s)\n", g_pairObjectLabel.second.c_str(), stdstrReqBody.c_str());
	...
	nRet = requestClient.Request(g_pairObjectLabel.first, g_pairObjectLabel.second, stdstrQuery, stdstrReqBody);
	if (nRet == 200)
	{
		// status ok
		m_stdstrResponse = requestClient.GetResponseData();

		nRet = RESTGOOGLECLOUDSERVICE_ERROR_SUCCESS;
	}
	else
	{
		nRet = m_nError = RESTGOOGLECLOUDSERVICE_ERROR_HTTP_STATUS;
...
vector<string> CKRestGoogleCloudService::GetLabelDescriptions()
{
	...
	if (!m_stdstrResponse.empty())
	{
		...
		{
			m_nError = RESTGOOGLECLOUDSERVICE_ERROR_INVALID_JSON;
...
vector<pair<string, double>> CKRestGoogleCloudService::GetLabelDescriptionsScore()
{
...
string CKRestGoogleCloudService::GetServiceMessage(int& nRetError)
{
	nRetError = m_nError;
...
char* CKRestGoogleCloudService::ConvertImageToBase64CharBuff(string stdstrImageFilePath)
{
	char* pRetBuff = NULL;
