...
BOOL CRedirector::Open(LPCTSTR pszCmdLine, LPTSTR pszParam)
{
	...
	__try
	{
		...
		if (!LaunchChild(pszCmdLine, pszParam,
...
BOOL CRedirector::LaunchChild(LPCTSTR pszCmdLine, LPTSTR pszParam,
							  HANDLE hStdOut,
							  HANDLE hStdIn,
							  HANDLE hStdErr)
{
	...
	// Launch the child process.
	if (!::CreateProcess(NULL, (LPTSTR)pszCmdLine, NULL, NULL, TRUE, CREATE_NEW_CONSOLE | CREATE_UNICODE_ENVIRONMENT, NULL, NULL, &si, &pi))
	//if (!::CreateProcess(pszCmdLine, pszParam, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi))
	{
		return FALSE;
	}
...
{
	m_mutexOutputBuffer.lock();

#ifdef _UNICODE
	wchar_t buffer[256];
	MultiByteToWideChar(CP_ACP, 0, pszOutput, -1, buffer, 256);
	mStdOutBuffer.Append(buffer);
#endif
	mStdOutBuffer.Append((CString)pszOutput);
	TRACE("%s", pszOutput);

	m_mutexOutputBuffer.unlock();
}
...
//void CRedirector::SetStdOutput(CString *stdoutBuffer)
//{
//	mStdOutBuffer = stdoutBuffer;
//}

CString CRedirector::GetResult(BOOL bCurrent)
{
	DWORD dwElapse = INFINITE;
	...
	::WaitForSingleObject(m_hThread, dwElapse);
...
void CRedirector::Abort(UINT nExitCode)
{
	if(m_hChildProcess)
	{
		TerminateProcess(m_hChildProcess, nExitCode);
		Close();
	}
}

void CRedirector::Abort(DWORD dwProcessID, UINT nExitCode)
{
	...
	if (hProcess != NULL)
	{
		BOOL result = TerminateProcess(hProcess, nExitCode);
		CloseHandle(hProcess);
	}
}

BOOL CRedirector::IsRunningChildProcess()
{
	if(!m_hChildProcess)
		return FALSE;

	DWORD dwRet = ::WaitForSingleObject(m_hChildProcess, 500);
