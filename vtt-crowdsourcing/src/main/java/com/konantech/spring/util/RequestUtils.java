...
public class RequestUtils {
    ...
    public static String getParameter(HttpServletRequest request, String name) {
        ...
        if (value != null) {
            try {
                ...
                if (contentType != null && contentType.length() > 0 && contentType.startsWith("multipart/form-data")) {
                    return value;
                } else {
    ...
    public static String getRequiredParameter(HttpServletRequest request, String name) throws ServletException {
        String value = getParameter(request, name);
        if (value == null) {
    ...
    public static void setStartAndEnd(Map<String, Object> paramMap, int fromDay, int toDay) {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        //날짜
        if (paramMap.get("start") == null || StringUtils.length(MapUtils.getString(paramMap, "start")) != 10) {
            if (fromDay == 0) {
                paramMap.put("start", simpleDateFormat.format(new Date()));
            } else {
                paramMap.put("start", simpleDateFormat.format(DateUtil.addDate(new Date(), Calendar.DATE, fromDay)));
            }
        }
        if (paramMap.get("end") == null || StringUtils.length(MapUtils.getString(paramMap, "end")) != 10) {
            if (toDay == 0) {
                paramMap.put("end", simpleDateFormat.format(new Date()));
            } else {
                paramMap.put("end", simpleDateFormat.format(DateUtil.addDate(new Date(), Calendar.DATE, toDay)));
            }
        }
    }
    ...
    public static Map getParameterMap(HttpServletRequest request) {
        ...
        while (enums.hasMoreElements()) {
            ...
            if(StringUtils.indexOf(parameters.toString(),"[") > -1 ) {
