...
(function($) {
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            updateSelection = function () {
                ...
                $selection.addClass('layer' + id);  //orig
                $selection.html(
                    '<span class=photo_layer_num>' + id + '</span>'  // orig
                    //'<span class="photo_layer_num">' + (id+1) + '</span>'
                );
            },
            ...
            updateBtDelete = function (visible) {
                if ($btDelete) {
                    $btDelete.css({
                        ...
                        top: area.y - $btDelete.outerHeight() + 27,
                        "z-index": area.z + 1,
                        "background" : "#ff490a"
                    });
                }
       ...
       /* $outline = $("<div class=\"select-areas-outline\" />")
        ...
        if (options.allowResize) {
            $resizeHandlers["se"] =  $("<div class=\"select-areas-resize-handler se\"/>")
                .addClass("ebox_"+id)  // gt
                .css({
                    position : "absolute",
                    cursor : "se-resize"
        ...
        if (options.allowDelete) {
            ...
            // gt :  delete-area 에 ebox_{id} 추가
            $btDelete = bindToDelete($("<div class=\"delete-area ebox_"+id+"\" />"))
    ...
    $.imageSelectAreas.prototype.remove = function (id) {
        if (this._areas[id]) {
            //console.log("<< imageSelectAreas.prototype.remove >> this.areas[id]="+id);
    ...
    $.imageSelectAreas.prototype.newArea = function (event) {
        ...
        //console.log("<< imageSelectAreas.prototype.newArea >> newArea id ="+id);
        return id;
    };

    $.imageSelectAreas.prototype.set = function (id, options, silent) {
        if (this._areas[id]) {
            options.id = "layer" + id;  // orig
