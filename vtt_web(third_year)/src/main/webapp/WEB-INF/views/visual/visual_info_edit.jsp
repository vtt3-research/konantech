    ...
    $(document).ready(function(){
                ...
                $("[name=faceForm]").each(function(){
                    ...
                    var person = {
                        ...
                        person_info: {
                            ...
                            predicate: $(this).find("[name=predicate]").val() || '',
                            emotion: {}
                        },
                        related_objects: []
                    };
        ...
        var makeHtml = {
            select: function(opts) {
                ...
                var select = $('<select>',{'class':'form-control', id: opts.selectId}).attr({name: opts.selectName}).appendTo(itemWrap);
        ...
        function addFaceInfo(area, objxystr, regImgYn) {
            ...
            if(RepImageYn){
                var select3 = makeHtml.select({
                    labelName: '감정', selectId: "faceEmotion_" +area.boxseq, selectName: 'faceEmotion', selectVal: area.emotion,
                    option: [
                ...
                var select4 = makeHtml.select({
                    labelName: '행동', selectId: "faceAction_" +area.boxseq, selectName: 'faceAction', selectVal: area.behavior,
                    option: [
            ...
            function setFaceComboNone(bool){
                ...
                if(bool){
                    form1.find('select[name=faceAction]').val('');
                    form1.find('select[name=faceAction]').next().find('.ui-autocomplete-input').val('');
                }
            }
        ...
        function addPlaceInfo(area, objxystr, regImgYn) {
            ...
            var select1 = makeHtml.select({
                labelName: '장소', selectId: "place_" +area.boxseq, selectName: 'place', selectVal: area.boxname,
                option: [
