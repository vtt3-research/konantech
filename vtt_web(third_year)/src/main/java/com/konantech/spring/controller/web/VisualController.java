        ...
        for(Map map : contentList){
            ...
            for(ShotTB shotData : shotlist){
                ...
                while(isLoop){
                    ...
                    tagParam.put("assetimgfilename", "IMAGE_"+ StringUtils.leftPad(Integer.toString(frameIdx), 10, "0")+".jpg");
...
//        int repPutCnt = 0;
//
//        log.debug("getPutRepreImg map ====> " + map);
//
//        for(String key : map.keySet()) {
//            String paramJson = map.get(key);
//            log.debug("key ===> " + key);
//            log.debug("paramJson ===> " + paramJson);
//            Map paramMap = mapper.readValue(paramJson, HashMap.class); // error
